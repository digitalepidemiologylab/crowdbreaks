class SubmitTasksJob < ApplicationJob
  ...
  rescue_from(StandardError) do |exception|
    ErrorLogger.error "[#{self.class.name}] #{exception}"
  end
  ...
  def perform(mturk_batch_job_id)
    ...
    if hittype_id.nil? || qualification_type_id.nil?
    ...
    mturk_batch_job.update_attributes!({ hittype_id: hittype_id, qualification_type_id: qualification_type_id })

    # exclude blacklisted workers
    if mturk_batch_job.exclude_blacklisted? && !mturk_batch_job.sandbox?
    ...
    num_hits = if mturk_batch_job.check_availability_before? || mturk_batch_job.check_availability_before_and_after?
                 mturk_batch_job.mturk_tweets.available.count * mturk_batch_job.number_of_assignments
               else
                 mturk_batch_job.tasks.count
               end
    mturk_batch_job.tasks.each_with_index do |t, ix|
      ...
      t.update_attributes!({ hit_id: hit.hit_id })
