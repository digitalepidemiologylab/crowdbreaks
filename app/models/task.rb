class Task < ApplicationRecord
  ...
  def update_on_final(tasks_params)
    ...
    if mturk_tweet.tweet_id.to_s != tasks_params[:tweet_id]
      ErrorLogger.error("Task #{id} done by worker #{tasks_params[:worker_id]} was for tweet #{mturk_tweet.tweet_id} and is now re-assigned to tweet #{tasks_params[:tweet_id]}. HIT id: In params: #{tasks_params[:hit_id]} in Task: #{hit_id}")
      self.update_attributes!({
